      ...
      <div className="dashboard__metrics">
        {[1, 2, 3, 4].map((_, index) => (
          <div key={index} className="dashboard__card">
            <div
              style={{
                backgroundColor: index === 0 ? '#4CAF50' : index === 1 ? '#2196F3' : index === 2 ? '#9C27B0' : index === 3 ? '#FF9800' : ''
              }}
              className="dashboard__card-icon">
              {index === 0 ? <DollarSign /> : index === 1 ? <Boxes /> : index === 2 ? <PackageCheck /> : index === 3 ? <Users /> : null}
            </div>
            <div className="dashboard__card-info">
              <h1 className="dashboard__card-value">{
                index === 0 ? `$${data?.totalSales}` :
                  index === 1 ? `${data?.totalOrders}` :
                    index === 2 ? `${data?.totalProducts}` :
                      index === 3 ? `${data?.totalCustomers}` : ''

              }</h1>
              <p className="dashboard__card-label">{
                index === 0 ? 'Total Sales' :
                  index === 1 ? 'Total Orders' :
                    index === 2 ? 'Products' :
                      index === 3 ? 'Customers' : ''
      ...
      <div className="dashboard__orders">
        <div className="dashboard__orders-header">
          ...
          <p
            ...
            className="dashboard__orders-link">View all</p>
        </div>
          ...
          <tbody className="dashboard__table-body">
            {data?.recentOrders?.length > 0 && data.recentOrders.map((order, index) => (
              <tr key={index} className="dashboard__table-row">
                ...
                <td className="dashboard__table-data">{order.shippingDetails.firstName + " " + order.shippingDetails.lastName}</td>
                <td className="dashboard__table-data">{new Date(order.Date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</td>
